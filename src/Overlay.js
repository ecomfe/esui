...
define(
    function (require) {
        ...
        Overlay.prototype.initStructure = function () {
            ...
            Panel.prototype.initStructure.call(this, arguments);
        };
        ...
        Overlay.prototype.repaint = require('./painters').createRepaint(
            ...
            },
            {
                ...
                paint: function (overlay, attachedDOM, attachedControl, attachedLayout) {
        ...
        Overlay.prototype.show = function () {
            ...
            if (this.autoClose) {
                // 点击文档自动关闭
                this.helper.addDOMEvent(document, 'mousedown', u.bind(close, this));
            ...
            globalResizeHandler = lib.curry(resizeHandler, this);
            if (this.fixed) {
                this.helper.addDOMEvent(window, 'resize', globalResizeHandler);
                this.helper.addDOMEvent(window, 'scroll', globalResizeHandler);
            }
        ...
        Overlay.prototype.hide = function () {
            if (this.isShown) {
                this.helper.removeDOMEvent(window, 'resize', globalResizeHandler);
                this.helper.removeDOMEvent(window, 'scroll', globalResizeHandler);
        ...
        function autoLayout() {
            ...
            // 有粘连元素
            if (!u.isUndefined(attachedTarget) && !u.isNull(attachedTarget)) {
                if (!u.isArray(attachedLayout)) {
        ...
        function getTargetDOM(domId, control) {
            // DOM优先
            if (!u.isUndefined(domId) && !u.isNull(domId)) {
                return lib.g(domId);
            }
            else if (!u.isUndefined(control) && !u.isNull(control)) {
                // 传的是id
        ...
        function renderLayer(options) {
            ...
            // 设置class
            if (properties.align && properties.align.length) {
                lib.addClass(
                    main,
            ...
            for (var name in properties) {
                if (!isUndefined(properties, name)) {
                    main.style[name] = properties[name] + 'px';
                }
            }
        }

        function isUndefined(properties, key) {
            if (arguments.length === 1) {
                return properties === null || lib.trim(properties) === '';

            }
            else if (arguments.length === 2) {
                return !properties.hasOwnProperty(key) || properties[key] === null || lib.trim(properties[key]) === '';
            }
        }
        ...
        Overlay.prototype.selfLayout = function (options) {
            ...
            // 如果左右都没配，则自动居中
            if (isUndefined(properties, 'left') && isUndefined(properties, 'right')) {
                properties.left = (page.getViewWidth() - layerPosition.width) / 2;
            }
            // 如果都配了，则计算出宽度，然后取消right的设置
            else if (!isUndefined(properties, 'left') && !isUndefined(properties, 'right')) {
                // 如果宽度没配，才计算
            ...
            // 如果上下都没配，则自动居中
            if (isUndefined(properties, 'top') && isUndefined(properties, 'bottom')) {
        ...
        var globalResizeHandler; // resize的全局句柄
